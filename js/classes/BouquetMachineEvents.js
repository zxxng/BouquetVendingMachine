class BouquetMachineEvents {
  ...
  saveItemGenerator(target) {
    ...
    saveItem.innerHTML = `
		<div>
			<img src="./assets/images/${target.dataset.img}" alt="${target.dataset.name}">
  ...
  bindEvent() {
      ...
      if (inputMoney) {
        if (inputMoney > myMoneyVal) {
          ...
          this.balance.textContent =
            new Intl.NumberFormat().format((balanceVal ? balanceVal : 0) + inputMoney) + '원';
          ...
          for (const i of saveListItem) {
            if (targetEl.dataset.name === i.dataset.name) {
              i.querySelector('strong').textContent =
                parseInt(i.querySelector('strong').textContent) + 1;
    ...
    this.btnGet.addEventListener('click', () => {
      ...
      for (const saveItem of saveItemList) {
        ...
        for (const getItem of getItemList) {
          if (saveItem.dataset.name === getItem.dataset.name) {
            // 획득한 상품 카운트 증가
            getItem.querySelector('strong').textContent =
              parseInt(getItem.querySelector('strong').textContent) +
              parseInt(saveItem.querySelector('strong').textContent);
      ...
      this.getList.querySelectorAll('li').forEach((getItem) => {
        totalPrice +=
          parseInt(getItem.dataset.price) * parseInt(getItem.querySelector('strong').textContent);
      });
